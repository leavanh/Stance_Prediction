...
def fit_bert(input_name, batches = 16, epochs = 11, lr = 2.95e-5): # defaults (except batch size) are from original project (Maximilian Witte)
  ...
  wandb.init(project="bert", name = str(input_name))
  ...
  # preprocess data
  tokenizer = AutoTokenizer.from_pretrained("bert-base-german-cased", cache = path + "cache")
  ...
  model = AutoModelForSequenceClassification.from_pretrained("bert-base-german-cased", num_labels=2, id2label=id2label, label2id=label2id)
  model = model.to(device)

  model_path = path + "Models/stance_prediction/" + f'bert_{str(input_name)}'
  ...
  print(f"{current_time}--- New model bert_{str(input_name)} saved with mean acc of {acc} % and f1 of {f1} %. Parameter {batches}, {lr}, {epochs} ---")
