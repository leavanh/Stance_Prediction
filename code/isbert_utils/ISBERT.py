...
class ISBERT(nn.Sequential):
    ...
    def __init__(self, model_name_or_path: str = None, modules: Iterable[nn.Module] = None, device: str = None):
        if model_name_or_path is not None and model_name_or_path != "":
            logging.info("Load pretrained ISBERT: {}".format(model_name_or_path))
                ...
                if not os.path.exists(model_path) or not os.listdir(model_path):
                    ...
                    try:
                        ...
                        if e.response.status_code == 404:
                            logging.warning('ISBERT-Model {} not found. Try to create it from scratch'.format(model_url))
            ...
            if model_path is not None:
                logging.info("Load ISBERT from folder: {}".format(model_path))
    ...
    def start_multi_process_pool(self, target_devices: List[str] = None, encode_batch_size: int = 32):
        ...
        for cuda_id in target_devices:
            p = ctx.Process(target=ISBERT._encode_multi_process_worker, args=(cuda_id, self, input_queue, output_queue, encode_batch_size), daemon=True)
            p.start()
            processes.append(p)
    ...
    def encode_multi_process(self, sentences: List[str], pool: Dict[str, object], is_pretokenized: bool = True, chunk_size=None):
        """
